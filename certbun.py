...
if len(sys.argv)>1: #at least the config and root domain is specified
	...
	try:
		if os.path.exists(apiConfig["domainCertLocation"]):
			f_check = open(apiConfig["domainCertLocation"], "r")
			existing_cert = f_check.read()
			f_check.close()

			if existing_cert.strip() == certJSON["certificatechain"].strip():
				print("Certs were not changed. Reload is not required.")
				sys.exit()
	except Exception as e:
		print("Certs were not compared, continue updating: " + str(e))
